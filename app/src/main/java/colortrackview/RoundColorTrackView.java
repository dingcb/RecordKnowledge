...
public class RoundColorTrackView extends View {
    ...
    public RoundColorTrackView(Context context) {
        this(context, null);
    }

    public RoundColorTrackView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void measureText() {
        ...
        mTextWidth = (int)( mPaint.measureText(mText))+2*mTextHeight;
    }
...
//        if (color==mTextChangeColor){
//            mPaint.setColor(Color.parseColor("#009688"));
//            RectF rect = new RectF(0, 0, width, height);
//            canvas.drawRoundRect(rect, height/2, height/2, mPaint);//第二个参数是x半径，第三个参数是y半径
//        }
    ...
    private void drawText_h(Canvas canvas, int color, int startX, int endX) {
        ...
        // right, bottom
        canvas.drawText(mText, mTextStartX+mTextHeight,
                getMeasuredHeight() / 2
                        - ((mPaint.descent() + mPaint.ascent()) / 2), mPaint);

        if (color==mTextChangeColor){
            mPaint.setColor(tabIndicatorColor);
            RectF rect = new RectF(0, 0, width, height);
            canvas.drawRoundRect(rect, height/2, height/2, mPaint);//第二个参数是x半径，第三个参数是y半径
